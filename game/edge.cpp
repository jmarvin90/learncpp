...
Edge::Edge(const Point& origin, const Point& termination)
    : m_origin{origin}, m_termination{termination} {};

const Point& Edge::get_origin() const { return m_origin; }

const Point& Edge::get_termination() const { return m_termination; }

bool operator==(Edge& e1, Edge& e2) {
  return ((e1.m_origin == e2.m_origin && e1.m_termination == e2.m_termination) |
          (e1.m_origin == e2.m_termination && e1.m_termination == e2.m_origin));
}

int Edge::x_diff() const { return (m_origin.get_x() - m_termination.get_x()); }

int Edge::y_diff() const { return (m_origin.get_y() - m_termination.get_y()); }
...
double Edge::y_intercept() const {
  return (m_origin.get_y() - (gradient() * m_origin.get_x()));
}

double Edge::length() const { return m_origin.distance_to(m_termination); }
...
int orientation(const Point& p1, const Point& p2, const Point& p3) {
  ...
  double val = ((edge_1.y_diff() * edge_2.x_diff()) -
